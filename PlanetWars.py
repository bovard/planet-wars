...
class Planet:
  ...
  def CanSafeTakeOver(self, turn):
    ships = -1
    #logging.debug('in CanSafeTakeOver')
    reinforcement = 0
    ...
    for i in range(1,turn+1):
      ...
      for p in self._neighbors[i]:
        ...
        if ships >= 0 or reinforcement:
          reinforcement = 1
          ships += p.GetReinforcingTroops(1, turn-i)
        #logging.debug('only '+repr(ships)+' left!')

    if ships >= 0:
      #logging.debug('one can TAKEOVER (leaving CanSafeTakeOver)')
      return 1
    #logging.debug('one cannot take over! (leaving CanSafeTakeOver)')
    return 0

  def CommitSafeTakeOver(self, turn):
    if not(self._owner[turn]==1):
      ships = -1*(self._num_ships[turn]+1)
      #logging.debug('in CommitSafeTakeOver')
      #logging.debug('there are '+repr(ships)+' left!')
      ...
      for i in range(1,turn+1):
        ...
        for p in self._neighbors[i]:
          for j in range(turn,i-1,-1):
            ...
            if ships >= 0:
              #logging.debug('sucess! leaving CommitSafeTakeOver')
              #logging.debug(repr(self._free_troops))
              return 1
      #logging.debug('failed! CommitSafeTakeOver')
      return 0
    else:
