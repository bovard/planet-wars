  ...
  for i in range(pw.MaxDistance()):
    ...
    for p in pw.MyPlanets(i):
      ...
      if free < 0:
        ...
        attack_reinforcements.append([p.PlanetID(),i+1,free,free/2*p.GrowthRate()])
        if free/2*p.GrowthRate() < worst:
          worst = free/2*p.GrowthRate()
    ...
    for p in pw.EnemyPlanets(i):
      ...
      if free < 0:
        #logging.info('Planet'+repr(p.PlanetID())+' made a request for '+repr(free)+' on turn '+repr(i+1))
        attack_reinforcements.append([p.PlanetID(),i+1,free-1,(free-1)/2*p.GrowthRate()])
        if (free-1)/2*p.GrowthRate() < worst:
          worst = (free-1)/2*p.GrowthRate()

    #respond to requests
    #logging.debug('Responding to Attack Reinforcement Requests')
    for j in range(0,int(worst)-2,-1):
      for p in pw.Planets():
        if len(attack_reinforcements)>0:
          for request in attack_reinforcements:
            if request[0]==p.PlanetID():
              if request[3]>j and request[3]<=j+1:
                if p.GetOwner(i)<2:
...
def AttackNeutrals(pw, launch_queue):
  ...
  for j in range(pw.GetGlobalFarthestEnemy()):
    for i in range(1,pw.MaxDistance()):
